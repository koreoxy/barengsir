<aside id="sidebarComponent"
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        <p class="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 mt-6">Keamanan & Audit</p>

        <!-- Audit & Log Aktivitas -->
        <a href="{{ route('superadmin.audit.index') }}" 
           class="group relative flex items-center px-3 py-2.5 rounded-xl transition-all duration-150
                  {{ request()->routeIs('superadmin.audit.index') ? 'bg-blue-50 text-blue-700 font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-blue-700' }}">
            @if(request()->routeIs('superadmin.audit.index'))
                <span class="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-blue-500 rounded-r-full shadow-md shadow-blue-500"></span>
            @endif
            <svg class="w-4 h-4 mr-3 {{ request()->routeIs('superadmin.audit.index') ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <span class="text-xs font-semibold">Audit & Log Aktivitas</span>
        </a>

        <!-- Database Backups -->
        <a href="{{ route('superadmin.backups.index') }}" 
           class="group relative flex items-center px-3 py-2.5 rounded-xl transition-all duration-150
                  {{ request()->routeIs('superadmin.backups.*') ? 'bg-blue-50 text-blue-700 font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-blue-700' }}">
            @if(request()->routeIs('superadmin.backups.*'))
                <span class="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-blue-500 rounded-r-full shadow-md shadow-blue-500"></span>
            @endif
            <svg class="w-4 h-4 mr-3 {{ request()->routeIs('superadmin.backups.*') ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4" />
            </svg>
            <span class="text-xs font-semibold">Database Backups</span>
        </a>
