...
@section('title', 'Edit Cabang Vendor')
    ...
    <div class="max-w-3xl">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50/30">
                <h3 class="font-bold text-slate-800">Form Edit Cabang</h3>
                <p class="text-xs text-slate-500 mt-1">Ubah data cabang <span class="font-bold text-slate-700 uppercase">{{ $branch->name }}</span> milik vendor <span class="font-bold text-slate-700 uppercase">{{ $vendor->name }}</span>.</p>
            </div>

            <form action="{{ route('superadmin.vendors.branches.update', [$vendor, $branch]) }}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="address" id="address" rows="3"
                              ...
                              placeholder="Alamat lengkap lokasi cabang...">{{ old('address', $branch->address) }}</textarea>
                    @error('address')
                        <p class="text-xs text-rose-600 font-bold mt-1.5 flex items-center gap-1">
                            <svg class="w-3.5 h-3.5 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                            {{ $message }}
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-6">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="is_active" value="1" {{ old('is_active', $branch->is_active) == '1' ? 'checked' : '' }} class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-slate-300">
                            <span class="ml-2.5 text-xs text-slate-700 font-bold">Aktif / Buka</span>
                        </label>
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="is_active" value="0" {{ old('is_active', $branch->is_active) == '0' ? 'checked' : '' }} class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-slate-300">
