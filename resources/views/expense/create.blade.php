            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-slate-100 p-8">
                <form action="{{ route('expense.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input id="title" name="title" type="text" class="block w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50/50 focus:ring-4 focus:ring-blue-500/20 focus:border-blue-600 focus:bg-white text-sm text-slate-800 focus:outline-none transition-all font-semibold" placeholder="Contoh: Gaji Karyawan Bulan Mei" value="{{ old('title') }}" required />
                        @error('title')
                    ...
                    <div>
                        ...
                        <div class="relative rounded-xl shadow-xs">
                            ...
                            <input id="amount" name="amount" type="number" step="0.01" class="block w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 bg-slate-50/50 focus:ring-4 focus:ring-blue-500/20 focus:border-blue-600 focus:bg-white text-sm text-slate-800 focus:outline-none transition-all font-black" placeholder="0" value="{{ old('amount') }}" required />
                        </div>
                        @error('amount')
                            <span class="text-xs text-rose-600 font-bold mt-1 block">{{ $message }}</span>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea id="note" name="note" rows="3" class="block w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50/50 focus:ring-4 focus:ring-blue-500/20 focus:border-blue-600 focus:bg-white text-sm text-slate-800 focus:outline-none transition-all font-medium" placeholder="Tulis catatan atau rincian tambahan di sini...">{{ old('note') }}</textarea>
                        @error('note')
                            <span class="text-xs text-rose-600 font-bold mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
