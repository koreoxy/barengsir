        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <div class="p-6 border-b border-gray-100 bg-gray-50/40">
                    <h3 class="font-bold text-gray-800 text-sm">Form Edit Data Cabang</h3>
                    <p class="text-xs text-gray-500 mt-1">Ubah data cabang <span class="font-bold text-gray-700 uppercase">{{ $branch->name }}</span>.</p>
                </div>

                <form action="{{ route('vendor.branches.update', $branch) }}" method="POST" class="p-8 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center gap-6">
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="radio" name="is_active" value="1" {{ old('is_active', $branch->is_active) == '1' ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 focus:ring-indigo-500 border-gray-300">
                                <span class="ml-2 text-xs text-gray-700 font-bold">Aktif / Buka</span>
                            </label>
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="radio" name="is_active" value="0" {{ old('is_active', $branch->is_active) == '0' ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 focus:ring-indigo-500 border-gray-300">
